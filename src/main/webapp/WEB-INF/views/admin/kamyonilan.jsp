		...
		<section class="content">
			<!-- Info boxes -->
					<h1>İlan Sayfası</h1>
							...
							<c:if test="${not empty uks }">

								<div id="myCarousel" class="carousel slide" data-ride="carousel">
									<!-- Indicators -->
									<ol class="carousel-indicators">
									<c:set var="count" value="1" scope="page" />
									<c:forEach items="${ uks }" var="item">
					...
					<h1>Satılık Kamyonet İlanı Sayfası</h1>
					  <form method="get" action='<s:url value="/kamyonet"></s:url>'>
     <style type="text/css">
...
<table class="table">
  ...
  <c:if test="${not empty vkamyonet }">
  
  
  <tr>
    
    <td class="tg-yw4l"><c:out value="${ vkamyonet.getVkbaslik() }"></c:out></td>
    <td class="tg-yw4l"><c:out value="${ vkamyonet.getVkacik() }"></c:out></td>
    <td class="tg-yw4l"><c:out value="${ vkamyonet.getVkdetay() }"></c:out></td>
    <td class="tg-yw4l"><c:out value="${ vkamyonet.getVkfiyat() }"></c:out></td>
     <td class="tg-yw4l"><c:out value="${ vkamyonet.getVkid() }"></c:out></td>
