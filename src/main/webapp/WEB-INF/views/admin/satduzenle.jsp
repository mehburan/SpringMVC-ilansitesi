 ...
 <jsp:include page="${request.contextPath}/menu"></jsp:include>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Satılık İlan Düzenleme Sayfası 
      </h1>
      
    <div class>
    <h3>İlan Başlığı</h3>
    <form autocomplete="off" action='<s:url value="/satduzenle"></s:url>' method="POST">
    
    <div class="input-group input-group-lg">
  <span class="input-group-addon" id="sizing-addon1">"Aa"</span>
  <input type="hidden" name="esid" value='<c:out value="${ es.getEsid() }"></c:out>'>
  <input  type="text" class="form-control" name="esbaslik" value='<c:out value="${ es.getEsbaslik() }"></c:out>' placeholder="İlan Başlığı" aria-describedby="sizing-addon1">
  </div>
  <h3>İlan Açıklaması</h3>
  	<div class="input-group input-group-lg">
  	<span class="input-group-addon" id="sizing-addon1">"Aa"</span>
  	<input type="text" name="esacik" value='<c:out value="${ es.getEsacik() }"></c:out>' class="form-control" placeholder="İlan Açıklaması" aria-describedby="sizing-addon1">
  	</div>
  	<h3>İlan Detayı</h3>
  	<div class="input-group input-group-lg">
  	<span class="input-group-addon" id="sizing-addon1">"Aa"</span>
  	<input type="text" name="esdetay" value='<c:out value="${ es.getEsdetay() }"></c:out>' class="form-control" placeholder="İlan Detayı" aria-describedby="sizing-addon1">
  	</div>
  	
<h3>İlan Fiyatı</h3>
  	<div class="input-group">
  <span class="input-group-addon">₺</span>
  <input name="esfiyat" type="text" value="${ es.getEsfiyat() }" class="form-control" aria-label="Amount (to the nearest dollar)">
...
<br/>
    ...
    <section class="content">
      <!-- Info boxes -->
      <form method="get" action='<s:url value="admin/satilik"></s:url>'>
      <div class="content-wrapper">
     <style type="text/css">
...
<table class="table">
  ...
  <tr>
    ...
    <td class="tg-yw4l"><c:out value="${ es.getEsresim() }"></c:out></td>
    <td class="tg-yw4l"><c:out value="${ es.getEsfiyat() }"></c:out></td>
    <td><a href='<s:url value="/satduzenle/${ es.getEsid() }"></s:url>' class="btn btn-warning">Resim Düzenle</a></td>
